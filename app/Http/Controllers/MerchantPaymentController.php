    ...
    public function index(Request $request)
    {
        ...
        if (!empty($paypalCurrencyPaymentMethod)) {
            ...
            $data['currencyCode'] = $merchant->currency->code;
                ...
                //get paystack Client Id from CurrencyPaymentMethod
              $paystack                = PaymentMethod::where(['name' => 'paystack'])->first(['id']);
              $paystackCurrencyPaymentMethod = CurrencyPaymentMethod::where(['currency_id' => $merchant->currency->id, 'method_id' => $paystack->id])->where('activated_for', 'like', "%deposit%")->first(['method_data']);
              if (!empty($paystackCurrencyPaymentMethod)) {
                  $data['clientId']     = json_decode($paystackCurrencyPaymentMethod->method_data)->client_id;
                  $data['currencyCode'] = $merchant->currency->code;
              }
