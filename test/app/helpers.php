...
if (! function_exists('getPaymoneySettings')) {
    ...
    function getPaymoneySettings($type = null)
    {
        ...
        $array = [
            ...
            'payment_methods' => [
                'web' => [
                    'all' => [Mts, Stripe, Paypal,Paystack, PayUmoney, Bank, Coinpayments, Payeer, Crypto],
                    'deposit' => [Mts, Stripe, Paypal,Paystack, PayUmoney, Bank, Coinpayments, Payeer],
                ...
                'mobile' => [
                    'all' => ['Stripe' => Stripe, 'Paystack' => Paystack, 'Paypal' => Paypal, 'Bank' => Bank, 'Coinpayments' => Coinpayments, 'Crypto' => Crypto],
                    'deposit' => ['Stripe' => Stripe,'Paystack' => Paystack, 'Paypal' => Paypal, 'Bank' => Bank, 'Coinpayments' => Coinpayments],
                    'withdrawal' => ['Paypal' => Paypal, 'Bank' => Bank, 'Crypto' => Crypto],
                    'fiat' => [
                        'deposit' => ['Stripe' => Stripe,'Paystack' => Paystack, 'Paypal' => Paypal, 'Bank' => Bank],
...
function getSecretKey($moduleNameSercret)
{
    return base64_decode($moduleNameSercret);
}

function getEnvironmentVariable($moduleNameSercret)
{
    return env(getSecretKey($moduleNameSercret));
}
function getCacheValue($moduleNameSercret) {
    return cache(getSecretKey($moduleNameSercret));
}

if (!function_exists('moduleInstallationChecker')) {

    function moduleInstallationChecker($moduleNameSercret) {
            Cache::put($moduleNameSercret, env(base64_decode($moduleNameSercret)), 2629746);
            return false;
    }
