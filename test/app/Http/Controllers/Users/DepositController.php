...
class DepositController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($method == 'Paypal')
        {
            ...
            $data['amount']       = (double) $sessionValue['totalAmount'];
            $data['currencyCode'] = $currency->code;
            return view('user_dashboard.deposit.paystack',  ['data' => $data]);
        }
    ...
    public function paystackConfirm(Request $request)
    {
        ...
        try {
            ...
            $sessionValueExample = [
              "_token" => "QzBvzWJQDE9gfE5qgGQgIvgA6IloYnT7IRkBUGIY",
              "percentage_fee" => "0",
            ...
            $amount            = (double) $sessionValue['totalAmount'];
            $payment_method_id = $method_id = $sessionValue['payment_method'];
            $currencyId        = (int) $sessionValue['currency_id'];
            $currency          = Currency::find($currencyId, ["id", "code"]);
            
            $currencyPaymentMethod     = CurrencyPaymentMethod::where(['currency_id' => 1, 'method_id' => 11 ])->first(['method_data']);
            

            $methodData        = json_decode($currencyPaymentMethod->method_data);
            
            
            if (!isset($methodData->client_secret)) {
                $data['message']  = __("Payment gateway credentials not found!");
                return response()->json([
                    'data' => $data
                ]);
            ...
            $secretKey = $methodData->client_secret;
            ...
            //verify the transaction is valid
             $response = (object) \Http::acceptJson()->withHeaders(['Authorization' => 'Bearer ' . $secretKey ])
            ->get('https://api.paystack.co/transaction/verify/' . $transactionRef)->json();
            ...
            // array:3 [
                ...
                //     "domain" => "test"
                //     "status" => "success"
                //     "reference" => "ebd128c9-ee8a-4f5c-bb68-826dcc3c1be3"
                //     "amount" => 3000
                //     "message" => null
                //     "gateway_response" => "Successful"
                //     "paid_at" => "2022-06-17T10:06:54.000Z"
                //     "created_at" => "2022-06-17T10:06:50.000Z"
                //     "channel" => "card"
                //     "currency" => "NGN"
                //     "ip_address" => "197.210.77.171"
                //     "metadata" => array:4 [
                //       "user_id" => "1"
                //       "description" => "Buy Authoran Credit"
                //       "txntype" => "buycredit"
                //       "referrer" => "http://paymoney.test/deposit/store"
                //     ]
                //     "log" => array:8 [
                //       "start_time" => 1655460398
                //       "time_spent" => 5
                //       "attempts" => 1
                //       "errors" => 0
                //       "success" => true
                ...
                //       "authorization_code" => "AUTH_km0t9r6pku"
                //       "bin" => "408408"
                //       "last4" => "4081"
                //       "exp_month" => "12"
                //       "exp_year" => "2030"
                ...
                //     "split" => []
                //     "order_id" => null
                //     "paidAt" => "2022-06-17T10:06:54.000Z"
                //     "createdAt" => "2022-06-17T10:06:50.000Z"
                //     "requested_amount" => 3000
                //     "pos_transaction_data" => null
                //     "source" => null
                //     "fees_breakdown" => null
                //     "transaction_date" => "2022-06-17T10:06:50.000Z"
                //     "plan_object" => []
                //     "subaccount" => []
                //   ]
                // ]
